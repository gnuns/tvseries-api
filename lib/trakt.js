    ...
    request(options, function (error, response, body) {
        if (!error && response.statusCode == 200) {
            if (body.length <= 1) {
                ...
                return getAlternativePoster(info.ids.imdb, function(err, poster) {
                  info.images.poster.full = poster;
                  return cb(null, info);
                });
            }
        } else {
...
function getAlternativePoster(id, cb) {
  request({
      url: 'http://www.omdbapi.com/?i='+id+'&plot=short&r=json&apikey='+config.omdbAPIKey,
